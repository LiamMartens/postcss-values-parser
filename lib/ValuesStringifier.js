...
module.exports = class ValuesStringifier extends Stringifier {
  /** @type {(
   ...
   referencePathResolver: (id: string) => string
   ) => ValuesStringifier}
  */
  static stringify(node, builder, referencePathResolver) {
    const stringifier = new ValuesStringifier(builder, referencePathResolver);
    stringifier.stringify(node);
  }

  /** @type {(builder: import('postcss').Builder, referencePathResolver: (id: string) => string) => ValuesStringifier} */
  constructor(builder, referencePathResolver = (id) => id) {
    super(builder);
  ...
  reference(node) {
    if (node.syntax === '{}') {
      this.basic(node, `{${this.referencePathResolver(node.value)}}`);
    } else if (node.syntax === '$') {
      this.basic(node, `$${this.referencePathResolver(node.value)}`);
    }
  }

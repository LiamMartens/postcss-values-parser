...
module.exports = class ValuesStringifier extends Stringifier {
  ...
  */
  static stringify(node, builder, referenceIdResolver) {
    const stringifier = new ValuesStringifier(builder, referenceIdResolver);
    stringifier.stringify(node);
  }

  /** @type {(builder: import('postcss').Builder, referenceIdResolver: (id: string) => string) => ValuesStringifier} */
  constructor(builder, referenceIdResolver = (id) => id) {
    super(builder);
  ...
  reference(node) {
    if (node.syntax === '{}') {
      this.basic(node, `{${this.referenceIdResolver(node.value)}}`);
    } else if (node.syntax === '$') {
      this.basic(node, `$${this.referenceIdResolver(node.value)}`);
    }
  }
